...
const lookupUUID = require("./lookupUUID");

module.exports = (name) =>
  new Promise(async function (resolve, reject) {
    // Make sure the username is a valid MC username
    if (!name.match(/\w{3,16}/g))
      return reject(`"${name}" is not a valid username.`);

    // Begin to scrape content;
    const $ = cheerio.load(
      await fetch(`https://namemc.com/search?q=${name}`).then((r) => r.text())
    );
    ...
    if (
      ...
      return reject("You are being rate limited. Retry after: 5 seconds.");
